...
if Meteor.isServer
  ...
  Tinytest.add "Match - By one stop and B point", (test) ->
    #Trips.remove({});
    ride = JSON.parse(Assets.getText("tests/integration/CarpoolServiceClientTest-ride.json"))
    ride.bTime = moment().toDate() # Monday
    Trips.insert(ride);
    d "Trips in DB:", _(Trips.find({}, sort: time: -1).fetch()).pluck("fromAddress", "toAddress");

    drive = JSON.parse(Assets.getText("tests/integration/CarpoolServiceClientTest-drive.json"))
