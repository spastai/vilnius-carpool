...
if Meteor.isServer
  ...
  trip = JSON.parse(Assets.getText("tests/integration/RecurrentTripTests-recurrent-trip.json"))
  trip.bTime = moment("2016-06-20T13:00:00").toDate() # Monday
  Trips.insert(trip);
  trip = JSON.parse(Assets.getText("tests/integration/RecurrentTripTests-trip.json"));
